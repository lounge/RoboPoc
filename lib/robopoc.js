...
class RoboPoc extends Bot {
  ...
  sendErrorMessage(data) {
    ...
    this.postMessageToChannel(this.channels[this.channel].name, data.title, { as_user: true, attachments: [{'color': 'danger', 'text': data.message}] });
  }

  sendSuccessMessage(data) {
    console.log('sendMessage success: ' + msg);
    this.postMessageToChannel(this.channels[this.channel].name, data.title, { as_user: true, attachments: [{'color': 'good', 'text': data.message}] });
  }
}
