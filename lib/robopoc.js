...
class RoboPoc extends Bot {
  ...
  sendMessages(data) {
    ...
    this.postMessageToChannel(this.channels[this.channel].name, data.title, { as_user: true, attachments: data.messages });
  }

  sendErrorMessage(data) {
    console.log('sendMessage error: ' + msg);
    this.postMessageToChannel(this.channels[this.channel].name, data.title, { as_user: true, attachments: [{'color': '#FF0000', 'text': data.message}] });
  }

  sendSuccessMessage(data) {
    console.log('sendMessage success: ' + msg);
    this.postMessageToChannel(this.channels[this.channel].name, data.title, { as_user: true, attachments: [{'color': '#00DB00', 'text': data.message}] });
  }
}
