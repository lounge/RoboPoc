...
class RoboPoc extends Bot {
  ...
  sendMessages(data) {
    console.log('sendMessages[] ' + this.channelName);
    console.log(data);
    this.postMessageToChannel(this.currentChannel.name, data.title, { as_user: true, attachments: data.messages });
  }

  sendErrorMessage(data) {
    console.log('sendMessage error: ' + data.message);
  ...
  sendSuccessMessage(data) {
    console.log('sendMessage success: ' + data.message);
