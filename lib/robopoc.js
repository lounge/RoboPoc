...
class RoboPoc extends Bot {
  ...
  sendErrorMessage(msg) {
    console.log('sendMessage error: ' + msg);
    this.postMessageToChannel(this.channels[this.channel].name, '', { as_user: true, attachments: [{'color': '#FF0000', 'text': msg}] });
  }

  sendSuccessMessage(msg) {
    console.log('sendMessage success: ' + msg);
    this.postMessageToChannel(this.channels[this.channel].name, '', { as_user: true, attachments: [{'color': '#00DB00', 'text': msg}] });
  }
}
