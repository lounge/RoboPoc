...
class RoboPoc extends Bot {
  ...
  _onStart() {
    ...
    this.started = true;
    this.currentChannel = this.getChannel();
  }
  ...
  sendMessage(msg) {
    ...
    this.postMessageToChannel(this.currentChannel.name, msg, { as_user: true });
  }
  ...
  sendMessages(data) {
    ...
    var channelName = this.currentChannel.name;
    this.postMessageToChannel(channelName, data.title, { as_user: true, attachments: data.messages });
  }

  sendErrorMessage(data) {
    console.log('sendMessage error: ' + msg);
    this.postMessageToChannel(this.currentChannel.name, data.title, { as_user: true, attachments: [{'color': 'danger', 'text': data.message}] });
  }

  sendSuccessMessage(data) {
    console.log('sendMessage success: ' + msg);
    this.postMessageToChannel(this.currentChannel.name, data.title, { as_user: true, attachments: [{'color': 'good', 'text': data.message}] });
  }
