...
class RoboPoc extends Bot {
  ...
  sendMessage(msg) {
    ...
    this.postMessageToChannel(this.getChannel().name, msg, { as_user: true });
  }

  sendMessages(data) {
    console.log('sendMessages[]');
    this.postMessageToChannel(this.getChannel().name, data.title, { as_user: true, attachments: data.messages });
  }

  sendErrorMessage(data) {
    console.log('sendMessage error: ' + msg);
    this.postMessageToChannel(this.getChannel().name, data.title, { as_user: true, attachments: [{'color': 'danger', 'text': data.message}] });
  }

  sendSuccessMessage(data) {
    console.log('sendMessage success: ' + msg);
    this.postMessageToChannel(this.getChannel().name, data.title, { as_user: true, attachments: [{'color': 'good', 'text': data.message}] });
  }
